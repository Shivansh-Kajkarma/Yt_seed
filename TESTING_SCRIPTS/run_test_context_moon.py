...
import sys
import json
import re  # <-- NEW: Import regex for safe filenames
...
def run_context_aware_retest():
    ...
    # ... (code for loading and fingerprinting Moon is unchanged)
    ...
    # ... (code for loading candidates is unchanged)
    try:
    ...
    for index, row in df_candidates_test.iterrows():
        ...
        # --- NEW CACHING LOGIC (with readable names) ---
        
        # 1. Create a safe, human-readable filename
        # (e.g., "The Young Turks" -> "The_Young_Turks_videos.csv")
        safe_name = re.sub(r'[^\w_.-]', '', cand_name.replace(" ", "_"))
        ...
        try:
            if os.path.exists(cache_file_path):
                ...
                if df_cand_videos.empty or 'channel_description' not in df_cand_videos.columns:
                    print(f"  ⚠️ Cache file {cache_file_path} is empty or invalid. Refetching.")
                    raise FileNotFoundError("Invalid cache file")
                    
                cand_desc = df_cand_videos['channel_description'].iloc[0]
                
            else:
                # 2. FETCH FROM API (and SAVE TO CACHE)
                print(f"  Fetching recent videos for {cand_name} (live API call)...")
                ...
                # --- Save to readable cache file! ---
                df_cand_videos.to_csv(cache_file_path, index=False)
                print(f"  ✅ Saved {len(df_cand_videos)} videos to cache: {cache_file_path}")

        except Exception as e:
            print(f"  ❌ Failed to fetch or load videos for {cand_name}: {e}")
            continue
        # --- END OF CACHING LOGIC ---
            
        # 3. Generate Candidate Fingerprint
        ...
        # ... (rest of fingerprinting logic is unchanged)
        ...
        results.append({
            ...
            # ... (rest of results dict is unchanged)
    ...
    # ... (rest of report logic is unchanged)
    if not results:
