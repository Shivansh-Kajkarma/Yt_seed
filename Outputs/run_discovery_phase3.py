...
#     df_deduped = df_with_duplicates.sort_values(
#         "Similarity_Score", ascending=False
#     ).drop_duplicates(subset=["Seed_Channel_ID", "Discovered_Channel_ID"], keep="first")
...
#         ["Seed_Channel_Name", "Similarity_Score"], ascending=[True, False]
#     )
...
#     print(f"\n🔍 STEP 1: Searching YouTube with {len(seed_keywords)} keywords...")

#     try:
#         candidate_ids = search_videos_multi_focused(
#             seed_keywords,
#             max_results_per_search=10,
#             max_keywords=len(seed_keywords)  # Use ALL keywords from the seed
#         )
#     except Exception as e:
...
#                 _update_status(
#                     seen_channels_data, candidate["id"], "skipped_few_videos"
#                 )
#                 continue
...
#             #Language filter
#             print(f"     Detecting language (Target: en)...")
...
#             # Add this mapping at top of file
#             LANGUAGE_NAMES = {
#                 'en': 'English',
#                 'hi': 'Hindi', 
#                 'es': 'Spanish',
...
#                 print(f"     ❌ FILTERED: Language not English (Detected: {lang_name} [{primary_language}])")

#                 _update_status(
#                     seen_channels_data, candidate["id"], f"filtered_lang_{primary_language}"
#                 )
#                 continue  # <-- Skip to the next candidate
            
#             print(f"     ✅ Language: {primary_language} (Passed)")
...
#                 _update_status(
#                     seen_channels_data, candidate["id"], "failed_fingerprint"
#                 )
#                 continue
...
#                 result_embeddings = {
#                     **result_base,
#                     "Similarity_Score": similarity_embeddings,
#                 }
#                 embedding_results.append(result_embeddings)
#                 print(
#                     f"     ✅ HIGH (Embeddings >= {SIMILARITY_THRESHOLD_EMBEDDINGS})!"
#                 )
...
#                 # Extract keywords from nested category structure
#                 if details.get("keywords"):
#                     all_keywords = []
#                     keywords_data = details["keywords"]

#                     # Check if it's a dict with categories
#                     if isinstance(keywords_data, dict):
#                         # Flatten all keywords from all categories
#                         for category, keyword_list in keywords_data.items():
#                             if isinstance(keyword_list, list):
#                                 all_keywords.extend(keyword_list)
#                     # If it's already a list (old format)
#                     elif isinstance(keywords_data, list):
#                         all_keywords = keywords_data

#                     seed_keywords_map[channel_name] = all_keywords
#                     print(
#                         f"  📌 {channel_name}: {len(all_keywords)} keywords from {len(keywords_data) if isinstance(keywords_data, dict) else 1} categories"
#                     )
...
#         if (
#             "Channel_Name" not in df_videos.columns
#             or "Channel_ID" not in df_videos.columns
#         ):
...
#         print(
#             f"✅ Embeddings (high): {len(df)} results → {high_similarity_embeddings_path.name}"
#         )
...
#         all_embedding_results, all_llm_results
#     )

#     # Save with duplicates
#     if not df_with_duplicates.empty:
#         df_with_duplicates.to_csv(
#             merged_with_duplicates_path, index=False, encoding="utf-8-sig"
#         )
#         print(
#             f"✅ Saved (with duplicates): {len(df_with_duplicates)} → {merged_with_duplicates_path.name}"
#         )
